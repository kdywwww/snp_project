...
twelve_dict = {"SPY": ['spy_data.csv'], 
               "USD/KRW": ['usdkrw_data.csv']}
log_dir = "/logs"
...
for symbol in twelve_dict.keys():
    try:
        ...
        # Load original data
        data_df = pd.read_csv(os.path.join(data_dir, filename), parse_dates=['datetime'], index_col='datetime')
        ...
        logger.info(f"[TwelveData {symbol}] Existing Data last_date: {last_date.strftime('%Y-%m-%d')}, Download start_date: {start_date_str}")
        ...
        logger.info(f"[TwelveData {symbol}] Successfully retrieved data: {retrieval_df.shape[0]} rows.")
        ...
        data_df = data_df.sort_index()
        logger.info(f"[TwelveData {symbol}] Total data after concat: {data_df.shape[0]} rows.")
        ...
        logger.info(f"[TwelveData {symbol}] Save Data: {os.path.join(data_dir, filename)}")
